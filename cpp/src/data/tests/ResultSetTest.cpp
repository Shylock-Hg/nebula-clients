...
TEST(ResultSetTest, Basic) {
    ...
    nebula::ResultSet r(nebula::ExecutionResponse{
        nebula::ErrorCode::SUCCEEDED, 0, std::make_unique<nebula::DataSet>(data)});

    EXPECT_EQ(std::vector<std::string>({"col1", "col2", "col3"}), r.keys());

    EXPECT_EQ(std::vector<nebula::Value>({4, 5, 6}), r.rowValues(1));

    EXPECT_EQ(std::vector<nebula::Value>({2, 5, 8}), r.colValues("col2"));
